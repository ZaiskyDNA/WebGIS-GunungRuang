...
const MapComponent = dynamic(() => import('../../components/MapTanggap'), { 
  ssr: false,
  loading: () => (
    <div className="h-full w-full bg-canvas animate-pulse rounded-3xl flex flex-col items-center justify-center border-2 border-dashed border-volcano-dark/20 text-volcano-dark/60 font-bold tracking-widest text-sm">
...
export default function TanggapDarurat() {
  const [poskos, setPoskos] = useState<EvacuationPoint[]>([]);
  ...
  return (
    <main className="min-h-screen py-8 md:py-12">
      <div className="yota-shell space-y-8">
        ...
        {/* ── HEADER MARUN ELEGAN ── */}
        <div className="yota-hero p-8 md:p-10 rounded-[32px] flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="relative z-10 bg-white p-6 rounded-3xl shadow-2xl text-center min-w-[220px] transform hover:scale-105 transition-transform">
             ...
             <div className="text-5xl font-black text-volcano-dark mb-1">{totalRefugees}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 bg-white p-4 rounded-[32px] shadow-lg border border-gray-100 flex flex-col h-[500px] md:h-[650px] relative">
            <div className="absolute top-8 left-8 z-[400] bg-white/95 backdrop-blur-sm p-4 rounded-2xl shadow-xl border border-volcano-dark/10">
              <h2 className="text-sm font-bold text-volcano-dark uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="lg:col-span-4 bg-white p-6 md:p-8 rounded-[32px] shadow-sm border border-gray-100 flex flex-col h-[650px]">
            <h2 className="text-lg font-bold text-volcano-dark mb-6 flex items-center gap-2 border-b border-gray-100 pb-4">
                ...
                return (
                  <div key={posko.id} className="p-5 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-md transition-all cursor-default">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="font-bold text-volcano-dark text-sm leading-tight pr-2">{posko.name}</h3>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full ${percentage >= 100 ? 'bg-red-500' : percentage > 80 ? 'bg-volcano-orange' : 'bg-volcano-main'}`}
